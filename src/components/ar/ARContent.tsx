  ...
  return (
    <div>
          ...
          {spotifySearching ? (
            ...
            <div className="flex flex-col gap-2 max-h-[350px] overflow-y-auto">
              {spotifyResults.map((artist) => {
                ...
                return (
                  <div key={artist.id} className={cn("flex items-center gap-3 p-3 rounded-lg border border-border", !alreadyAdded && !adding && "cursor-pointer hover:bg-accent/50")} onClick={() => { if (!alreadyAdded && !adding) handleAddFromSpotify(artist); }}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground truncate">{artist.genres.slice(0, 3).join(", ") || "No genres"}</p>
                    </div>
                    {alreadyAdded ? (
                      <div className="flex items-center gap-1 text-xs text-muted-foreground"><Check className="h-3.5 w-3.5" /> Added</div>
                    ) : adding ? (
                      <Loader2 className="h-4 w-4 animate-spin text-muted-foreground" />
                    ) : (
                      <Button size="sm" variant="outline" onClick={(e) => { e.stopPropagation(); handleAddFromSpotify(artist); }}>Add</Button>
                    )}
                  </div>
                );
              })}
      ...
      <div className="flex-1">
        {view === "board" ? (
          <PipelineBoard prospects={filtered} onSelect={(id) => setSelectedProspectId(id)} onStageChange={(id, stage) => { updateProspect.mutate({ id, stage } as any); toast.success("Stage updated"); }} onDelete={(id) => { deleteProspect.mutate(id, { onSuccess: () => toast.success("Prospect deleted") }); }} />
        ) : (
          <ProspectTable prospects={filtered} onSelect={(id) => setSelectedProspectId(id)} onDelete={(id) => { deleteProspect.mutate(id, { onSuccess: () => toast.success("Prospect deleted") }); }} />
        )}
      </div>
